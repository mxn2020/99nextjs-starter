...
import { ChartContainer, ChartTooltip, ChartTooltipContent, ChartLegend, ChartLegendContent, type ChartConfig } from "@/components/ui/chart"; // Assuming this is installed at ui/chart
...
// Alternative direct HSL values matching common themes:
// const COLORS = ['hsl(221.2, 83.2%, 53.3%)', 'hsl(142.1, 70.6%, 45.3%)', 'hsl(34.9, 91.5%, 55.1%)', 'hsl(0, 84.2%, 60.2%)', 'hsl(262.1, 83.3%, 57.8%)'];


const dailySignupsChartConfig = {
  users: {
    label: "New Users",
    color: "hsl(var(--chart-1))", // Or a direct color like '#8884d8'
  },
} satisfies ChartConfig;

const userRoleChartConfig = {
  // Dynamic keys will be added based on roles, e.g., 'admin', 'user'
  // We will define labels and colors dynamically or use a mapping
} satisfies ChartConfig;


export default function AnalyticsDashboard({ analyticsData, error }: AnalyticsDashboardProps) {
    if (error) {
        return <p className="text-destructive p-4 bg-destructive/10 rounded-md">Error loading analytics data: {error}</p>;
    }
    if (!analyticsData) {
        return <p className="text-muted-foreground p-4 bg-muted/10 rounded-md">No analytics data available or still loading...</p>;
    }
    ...
    // Dynamically create ChartConfig for roles
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
                <Card className="col-span-1 lg:col-span-2">
                    ...
                    <CardContent className="min-h-[350px] pl-2"> {/* Added min-h and pl for better axis rendering */}
                        {formattedDailySignups.length > 0 ? (
                            <ChartContainer config={dailySignupsChartConfig} className="h-full w-full">
                                <LineChart accessibilityLayer data={formattedDailySignups} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                    ...
                                    <Line dataKey="users" type="monotone" stroke="var(--color-users)" strokeWidth={2} dot={{ r: 4, fill: "var(--color-users)", strokeWidth: 0 }} activeDot={{r: 6, strokeWidth: 1, fill: "var(--color-background)", stroke: "var(--color-users)"}}/>
                ...
                <Card>
                         ...
                         {formattedUserRoleDistribution.length > 0 ? (
                            <ChartContainer config={dynamicUserRoleChartConfig} className="mx-auto aspect-square max-h-[300px]">
                                <ActualPieChart accessibilityLayer>
                                    ...
                                    <Pie data={formattedUserRoleDistribution} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} innerRadius={60} labelLine={false} 
                                    ...
                                    >
                                        {formattedUserRoleDistribution.map((entry, index) => (
                                            <Cell key={`cell-${index}`} fill={entry.fill} stroke="hsl(var(--background))" style={{ outline: 'none' }} />
                                        ))}
                                    </Pie>
                 ...
                 <Card>
                    ...
                    <CardContent className="min-h-[350px] flex flex-col justify-center space-y-2">
                       ...
                       <p className="text-xs text-muted-foreground">More detailed table or list can go here.</p>
                       {/* TODO: Consider adding a table for role distribution here if pie chart is too cluttered or for more detail */}
                    </CardContent>
                </Card>
            </div>
        </div>
