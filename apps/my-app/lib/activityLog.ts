...
"use server";
import { getAdminClient } from '@/lib/supabase/server'; // Use admin client to bypass RLS for logging
...
export async function logUserActivity(
  ...
  try {
    ...
    const ipAddress = (headerStore.get('x-forwarded-for') || headerStore.get('x-real-ip') || 'unknown')?.split(',')[0]?.trim() || 'unknown';
