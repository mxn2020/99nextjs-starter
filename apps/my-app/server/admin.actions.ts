...
async function verifyAdminRoleAndGetActor(supabaseClient: Awaited<ReturnType<typeof getServerClient>>) {
...
    const supabase = await getServerClient(); // For admin verification
    const supabaseAdmin = getAdminClient(); // For creating user
...
export async function getAdminAnalyticsData(): Promise<{ data: AdminAnalyticsData | null; error?: string }> {
    ...
    try {
        ...
        const analyticsData: AdminAnalyticsData = {
            ...
            dailySignups: (dailySignupsRes.data as DailySignup[] ?? []).map(d => ({...d, signup_date: d.signup_date?.split('T')[0] || ''})) || [] as DailySignup[],
